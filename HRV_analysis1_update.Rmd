...
e4.ssa <- ssa(L=15, x=filter_time_day1_e4.clean$e4_rr, kind="toeplitz-ssa", svd.method = "svd")
summary(e4.ssa)
# 1st stage visual information for grouping
plot(e4.ssa) # scree plot of singular values to identify trends and pairings.  
# plot(e4.ssa, "series", groups=as.list(1:4)) # Reconstructed time series for first few components/projections
# 1st stage reconstruction
# plot(e4.ssa, type="paired") #3&4, 5&6
...
# Reconstructed plot
ggplot(df_e4, aes(x=Time, y=Reconstructed)) + geom_line(color="mediumturquoise") + ylab("Reconstructed RR-interval in ms") 
```
...
# plot(fb.ssa, type="paired") #3&4, 5&6, 6&7 
...
# Reconstructed plot
ggplot(df_fb, aes(x=Time, y=Reconstructed)) + geom_line(colour="mediumturquoise") + ylab("Reconstructed RR-interval in ms") + xlab("Time")
```
...
# plot(mb.ssa, type="paired") # 3&4, 5&6, 8&9
...
```
# Analyze HRV via the time domain, RMSSD, most relevant and accurate measure of ANS over the short-term
...
RMSSD is correlated with HF, both are measures of parasympathetic activity.
...
HRV, especially the high frequency components, is regulated by parasympathetic activity from the vagus nerve; only parasympathetic action can mediate the rapid changes accompanying such high frequency variation.

```{r}
# transform <- fft(df_mb$Reconstructed)
```