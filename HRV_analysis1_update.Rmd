...
Huge spike of ECG at, 14:19:58, >37,000ms. This is due to remove previous short rr-intervals (rows 4081-4177) and adding it to the rr-interval at row 4178. 
...
for (i in 1:(nrow(df_window)-1)){
  # select rr-interval for E4
  e4 <- filter_time_day1_e4.clean %>%
    filter(strftime(readable_timestamp, "%H:%M:%S")>=df_window$time[i] & strftime(readable_timestamp, "%H:%M:%S") <df_window$time[i+1])
  # select rr-interval for firstbeat
  fb <- filter_time_day1_fb.clean %>%
  filter(strftime(readable_timestamp, "%H:%M:%S")>=df_window$time[i] & strftime(readable_timestamp, "%H:%M:%S") < df_window$time[i+1])
  # select rr-interval for MSband
  mb <- filter_time_day1_mb.clean %>%
  filter(strftime(readable_timestamp, "%H:%M:%S")>=df_window$time[i] & strftime(readable_timestamp, "%H:%M:%S") < df_window$time[i+1])
  # calculate RMSSD for each e4 interval
  df_window$RMSSD_e4[i] <- HRV(e4$e4_rr)
  # calculate RMSSD for each firstbeat interval
  df_window$RMSSD_fb[i] <- HRV(fb$fb_rr)
  # calculate RMSSD for each firstbeat interval
  df_window$RMSSD_mb[i] <- HRV(mb$mb_rr)
}
